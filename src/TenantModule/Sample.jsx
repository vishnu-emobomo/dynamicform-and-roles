...
const LineItems = () => {
  ...
  const handleAddRow = async () => {
    ...
    console.log(updatedLineItem,"for form line item");
      ...
      const filesPayload = fileData.map(file => ({
        file: file.file,
        PK: file.PK,
        SK: file.SK,
        Name: file.Name[0]?.fileName,
      }));

      console.log(filesPayload,"the structure ")
    ...
    try {
      ...
      if (response.data.message === "LineItem updated successfully") {
        ...
        if (filesPayload.length > 0) {
          try {
            // Step 1: Send files to the server to get the signed URLs
            const uploadPromises = filesPayload.map(file => {
              const formData = new FormData();
              console.log(file.Name);
    formData.append("files", file.Name );
              formData.append("sortKey", file.SK);
              formData.append("PK", file.PK);
        
              console.log("Sending FormData:", formData);
              return apiClient.post("/api/s3-file/s3-lineitems-upload", formData);
            });
        
            // Wait for all file upload requests to get signed URLs
            const uploadResponses = await Promise.all(uploadPromises);
            
            // Assuming the response contains the signed URLs
            const signedUrls = uploadResponses[0]?.data?.files || []; // Get the files object with signed URLs
        
            if (signedUrls.length === 0) {
              throw new Error("No signed URLs returned from the server.");
            }
        
            // Step 2: Upload the actual files to S3 using the signed URLs
            for (const file of filesPayload) {
              const signedUrlObj = signedUrls.find(item => item.fileName === file.Name[0]?.fileName);
        
              if (!signedUrlObj) {
                throw new Error(`No signed URL found for file: ${file.Name[0]?.fileName}`);
              }
        
              const { signedUrl } = signedUrlObj;
        
              // Step 3: Upload the file to the S3 bucket using the signed URL
              ...
              console.log(`File ${file.Name[0]?.fileName} uploaded successfully.`);
            }
