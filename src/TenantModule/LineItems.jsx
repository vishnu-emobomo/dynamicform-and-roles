...
import LineAttachments from "../AdditionalComponent/LineAttachments";
...
const LineItems = () => {
  ...
  const [fileLineData, setFileLineData] = useState({
    filesLine: [], 
  })
  ...
  const onFilesLineAdded = (newFiles) => {
    setFileLineData((prevFileLineData) => ({
      ...prevFileLineData,
      filesLine: newFiles, 
    }));
  ...
  // console.log(fileLineData,": data for file line ")
  ...
  const handleAddRow = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://z0qexibhsk.execute-api.ap-south-1.amazonaws.com/Dev/api/line-item/update-lineitem`,
        updatedLineItem,
        {
          headers: {
      ...
      if (response.data.message === "LineItem updated successfully") {
        const sortKey = response.data.PK;

      

        if (fileLineData.filesLine && fileLineData.filesLine.length > 0) {
          try {
            const response = await apiClient.post("/api/s3-file/uploadfile", {
              files: fileLineData.filesLine.map((fileLine) => ({ fileName: fileLine.name })),
              sortKey: sortKey,
              Name: fileLineData.filesLine.map((fileLine) => ({ fileName: fileLine.name })),
              Type: "Project_LineItems",
            });
        
            console.log(response," : response from the line item submit")
            const { files: signedUrls } = response.data;

            console.log(signedUrls)
        
            for (const fileLine of fileLineData.filesLine) {

              console.log(fileLine)
              console.log(fileLineData)

              const signedUrlObj = signedUrls.find((item) => item.fileName === fileLine.name);


        
              if (!signedUrlObj) {
                throw new Error(`No signed URL found for file: ${fileLine.name}`);
              }
        
              const { signedUrl } = signedUrlObj;
        
               axios.put(signedUrl, fileLine, {
                headers: {
                  "Content-Type": fileLine.type,
                },
              });
        
              console.log(`File ${fileLine.name} uploaded successfully.`);
            }
          } catch (error) {
        ...
        alert("Failed to update line item: " + response.data.message); 
      }

      alert("successfully updated the project line item")
    ...
    try {
      ...
      if (response.data && response.data.message) {
        ...
        if (fileData.files && fileData.files.length > 0) {
          try {
            const uploadResponse = await apiClient.post("/api/s3-file/uploadfile", {
            ...
            const { files: signedUrls } = uploadResponse.data;
            console.log(fileData.files, " before the loop entry");
  
            const uploadPromises = fileData.files.map(async (file) => {
              const signedUrlObj = signedUrls.find((item) => item.fileName === file.name);
              console.log(signedUrlObj, ": the data from the project file data signedUrl");
      ...
      >
        ...
        <Grid item xs={12} md={9}>
          ...
          >
            ...
            <Grid item xsfileLisget={12} md={9}>
...
const cleanFileName = decodedFileName.replace(/^C#1232\/(Tender|Project)\//, '');
                ...
                >
                  ...
                  <DialogContent>
                    <Grid container spacing={3}>
                      ...
                      <Grid item xs={12}>
                        <LineAttachments 
                         onFilesLineAdded ={onFilesLineAdded} 
                        existingFilesLine ={filesLine} />
                      </Grid>
  ...
  <TableBody>
    {selectedItem.files
      ...
      (file) =>
        file.Location &&
        Object.values(file.Location).some((url) => url) 
    )
    .map((file, fileIndex) => (
      <TableRow key={fileIndex}>
        <StyledTableCell>
          {file.Location &&
            Object.values(file.Location)
            .filter((url) => url) 
            .map((url, urlIndex) => {
        ...
        <StyledTableCell>
          {file.Location &&
            Object.values(file.Location).map((url, urlIndex) => (
              <div key={urlIndex}>
                <DeleteSweepIcon
                  onClick={() => handleDelete(file.PK, file.SK, url)}
